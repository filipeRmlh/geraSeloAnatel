...
var labelSize=document.querySelector("#labelSize");
var labelOp=document.querySelector("#labelOpacity");
var selo;
var dataContainer = document.querySelector(".sidebar");
...
setValues=function(el,el2,el3){
    ...
    //Size
    selo.setW(el3.value);
    labelSize.innerHTML = Math.trunc(selo.width)+"px X "+Math.trunc(selo.height)+"px";
    valid[2]=true;
    //BG
    valid[3]=true;
    selo.bgOpacity=el4.value/100;
    labelOp.innerHTML = Math.trunc(selo.bgOpacity*100)+"%";
    selo.draw();
    validateDownload();
}
...
document.querySelector("#openSidebar").addEventListener("click",toggleDataContainer);
setValues();

function toggleDataContainer(){
    console.log("foi");
    var toggled = dataContainer.getAttribute("data-toggle");
    if(toggled&&toggled=='true'){
        dataContainer.setAttribute("data-toggle",false);
    }else{
        dataContainer.setAttribute("data-toggle",true);   
    }
}